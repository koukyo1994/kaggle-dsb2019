...
class PastSummary3TimeEncoding(Feature):
    ...
    for sess_id, sess in user_sample.groupby("game_session", sort=False):
        ...
        if sess_type == "Assessment" and (test or len(sess) > 1):
            ...
            features["month_sin"] = np.sin(
                2 * np.pi * sess["month"].iloc[0] / 12)
            features["month_cos"] = np.cos(
                2 * np.pi * sess["month"].iloc[0] / 12)
            features["year"] = sess["year"].iloc[0]
            features["hour_sin"] = np.sin(
                2 * np.pi * sess["hour"].iloc[0] / 24)
            features["hour_cos"] = np.cos(
                2 * np.pi * sess["hour"].iloc[0] / 24)
            features["dayofweek_sin"] = np.sin(
                2 * np.pi * sess["dayofweek"].iloc[0] / 7)
            features["dayofweek_cos"] = np.cos(
                2 * np.pi * sess["dayofweek"].iloc[0] / 7)
            features["weekofyear_sin"] = np.sin(
                2 * np.pi * sess["weekofyear"].iloc[0] / 52)
            features["weekofyear_cos"] = np.cos(
                2 * np.pi * sess["weekofyear"].iloc[0] / 52)
