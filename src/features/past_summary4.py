...
def past_summary_features(
    ...
    action_include_click = {
        "Mushroom Sorter (Assessment)": [4020, 4070],
        "Bird Measurer (Assessment)": [4025, 4070],
        "Cauldron Filler (Assessment)": [4020, 4070],
        "Cart Balancer (Assessment)": [4020, 4070],
        "Chest Sorter (Assessment)": [4020, 4025, 4070]
    }
    ...
    for sess_id, sess in user_sample.groupby("game_session", sort=False):
        ...
        if sess_type == "Game":
            ...
            summary["count_4020"] = (sess["event_code"] == 4020).sum()
            summary["count_4070"] = (sess["event_code"] == 4070).sum()
            summary["count_4020_ratio"] = \
                summary["count_4020"] / (
                    summary["count_4020"] + summary["count_4070"] + 1)
            ...
            action_include_click_diff = sess[sess["event_code"].isin(
                [4020, 4025, 4070])]["timestamp"].diff().map(
                    lambda x: x.seconds).fillna(0).tolist()
            action_include_click_diff = list(
                filter(lambda x: x != 0.0, action_include_click_diff))
            summary["mean_action_time"] = np.mean(action_diff)
            summary["mean_action_click_time"] = np.mean(
                action_include_click_diff)
            summary["action_click_time_by_action_time"] = \
                summary["mean_action_click_time"] / (
                    summary["mean_action_time"] + 1e-7)
        ...
        if sess_type == "Assessment" and (test or len(sess) > 1):
            ...
            if len(past_assess_summary[sess_title]) > 0:
                ...
                features["mean_action_click_time_same_assess"] = np.mean(
                    collect(same_assessments, "mean_action_click_time"))
                features["var_action_click_time_same_assess"] = np.var(
                    collect(same_assessments, "mean_action_click_time"))
                features["mean_var_action_click_time_same_assess"] = np.mean(
                    collect(same_assessments, "var_action_click_time"))
                features[
                    "mean_action_click_time_by_action_time_same_assess"] = \
                    np.mean(
                        collect(
                            same_assessments,
                            "action_click_time_by_action_time"))
                ...
                features["mean_action_click_time_last_same_assess"] = \
                    same_assessments[-1]["mean_action_click_time"]
                features["var_action_click_time_last_same_assess"] = \
                    same_assessments[-1]["var_action_click_time"]
                features[
                    "action_click_time_by_action_time_last_same_assess"] = \
                    same_assessments[-1]["action_click_time_by_action_time"]
            else:
                ...
                features["mean_action_click_time_same_assess"] = -1.0
                features["var_action_click_time_same_assess"] = -1.0
                features["mean_var_action_click_time_same_assess"] = -1.0
                features[
                    "mean_action_click_time_by_action_time_same_assess"] = -1.0
                ...
                features["mean_action_click_time_last_same_assess"] = -1.0
                features["var_action_click_time_last_same_assess"] = -1.0
                features[
                    "action_click_time_by_action_time_last_same_assess"] = -1.0
            ...
            for assess in assessments:
                ...
                if len(summs) > 0:
                    ...
                    features[assess + "_mean_action_click_time"] = np.mean(
                        collect(summs, "mean_action_click_time"))
                    features[assess + "_var_action_click_time"] = np.var(
                        collect(summs, "var_action_click_time"))
                    features[assess + "_mean_var_action_click_time"] = np.mean(
                        collect(summs, "var_action_click_time"))
                    features[assess + "_action_click_time_by_action_time"] = \
                        np.mean(
                            collect(summs, "action_click_time_by_action_time"))
                    features[assess + "_4070_mean"] = np.mean(
                        collect(summs, "4070"))
                    features[assess + "_4070_var"] = np.var(
                        collect(summs, "4070"))
                    features[assess + "_4020_mean"] = np.mean(
                        collect(summs, "4020"))
                    features[assess + "_4020_var"] = np.var(
                        collect(summs, "4020"))
                    features[assess + "_4020_ratio"] = np.mean(
                        collect(summs, "4020_ratio"))
                    ...
                    features[assess + "_mean_action_click_time"] = -1.0
                    features[assess + "_var_action_click_time"] = -1.0
                    features[assess + "_mean_var_action_click_time"] = -1.0
                    features[assess +
                             "_action_click_time_by_action_time"] = -1.0
                    features[assess + "_4070_mean"] = -1.0
                    features[assess + "_4070_var"] = -1.0
                    features[assess + "_4020_mean"] = -1.0
                    features[assess + "_4020_var"] = -1.0
                    features[assess + "_4020_ratio"] = -1.0
                    if assess == "Cauldron Filler (Assessment)":
            ...
            summary_["4020"] = (sess["event_code"] == 4020).sum()
            summary_["4020_ratio"] = summary_["4020"] / (
                summary_["4020"] + summary_["4070"] + 1)

            action_diff = sess[sess["event_code"].isin(
                action_code[sess_title])]["timestamp"].diff().map(
                    lambda x: x.seconds).fillna(0).tolist()
            action_diff = list(filter(lambda x: x != 0.0, action_diff))
            action_click_diff = sess[sess["event_code"].isin(
                action_include_click[sess_title])]["timestamp"].diff().map(
                    lambda x: x.seconds).fillna(0).tolist()
            action_click_diff = list(
                filter(lambda x: x != 0.0, action_click_diff))
            summary_["mean_action_time"] = np.mean(action_diff)
            summary_["var_action_time"] = np.var(action_diff)
            summary_["mean_action_click_time"] = np.mean(action_click_diff)
            summary_["var_action_click_time"] = np.var(action_click_diff)
            summary_["action_click_time_by_action_time"] = \
                np.mean(action_click_diff) / np.mean(action_diff)
            if len(all_attempts) > 0:
            ...
            for game in games:
                ...
                features["count_4020_" + game] = 0.0
                features["mean_4020_" + game] = 0.0
                features["var_4020_" + game] = 0.0
                features["mean_4020_ratio_" + game] = 0.0
                features["last_4020_ratio_" + game] = 0.0
                features["mean_action_time_" + game] = 0.0
                features["var_action_time_" + game] = 0.0
                features["mean_action_click_time_" + game] = 0.0
                features["var_action_click_time_" + game] = 0.0
                features["mean_action_click_time_by_action_time_" + game] = 0.0
                features["last_action_click_time_by_action_time_" + game] = 0.0
            ...
            for game, summ in past_game_summarys.items():
                ...
                features["var_4070_" + game] = np.var(
                    collect(summ, "count_4070"))
                features["count_4020_" + game] = sum(
                    collect(summ, "count_4020"))
                features["mean_4020_" + game] = np.mean(
                    collect(summ, "count_4020"))
                features["var_4020" + game] = np.mean(
                    collect(summ, "count_4020"))
                features["mean_4020_ratio_" + game] = np.mean(
                    collect(summ, "count_4020_ratio"))
                features["last_4020_ratio_" + game] = collect(
                    summ, "count_4020_ratio")[-1]
                features["mean_action_time_" + game] = np.mean(
                    collect(summ, "mean_action_time"))
                features["var_action_time_" + game] = np.var(
                    collect(summ, "mean_action_time"))
                features["mean_action_click_time_" + game] = np.mean(
                    collect(summ, "mean_action_click_time"))
                features["var_action_click_time_" + game] = np.var(
                    collect(summ, "mean_action_click_time"))
                features["mean_action_click_time_by_action_time_" + game] = \
                    np.mean(collect(summ, "action_click_time_by_action_time"))
                features["last_action_click_time_by_action_time_" + game] = \
                    collect(summ, "action_click_time_by_action_time")[-1]
