...
def ks_sampling(train, test, threshold=0.05):
    list_p_value = []
...
def model_pipeline(train, test, folds, features, cat_features, target, 
    ...
    with timer('Drop useless columns'):
        counter_cols = [c for c in train.columns if c.find('Counter')>=0]
        counter_cols = [c for c in counter_cols if not c.find('Ratio')>=0]
        features = [c for c in features if c not in counter_cols]
        ks_drop_feats = ks_sampling(train.loc[folds[folds.fold>=0].index], test, threshold=0.05)
        logger.info(f'len(ks_drop_feats): {len(ks_drop_feats)}')
        features = [c for c in features if c not in ks_drop_feats]
