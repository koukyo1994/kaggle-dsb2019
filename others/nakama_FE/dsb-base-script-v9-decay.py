            ...
            for i in range(len(tmp_df)):
                if i==0:
                    ...
                    if len(tmp_past)==0:
                        ...
                        nunique_df = pd.DataFrame(tmp_past[nunique_cols].nunique()).T.add_prefix('nunique_')
                        for c in sum_cols:
                    ...
                    _tmp_past = tmp[tmp.timestamp < tmp_df.loc[i, 'timestamp']]
                    tmp_past = _tmp_past[tmp_df.loc[i-1, 'timestamp'] <= _tmp_past.timestamp]
                    if len(tmp_past)==0:
                        sum_df = pd.concat([sum_df, pd.DataFrame({'No_playing_logs': [1]})], axis=0)
                    else:
                        nunique_df = pd.DataFrame(_tmp_past[nunique_cols].nunique()).T.add_prefix('nunique_')
                        days = (tmp_df.loc[i, 'timestamp'] - tmp_df.loc[i-1, 'timestamp']) / (24 * 60**2 * 10**9)
                        for key in _sum_df:
                            _sum_df[key] = decaying_counter(_sum_df[key], days)
                        for c in sum_cols:
                            _sum_df.update(Counter(tmp_past[c].values))
                        concat_df = pd.concat([nunique_df, 
                                               pd.DataFrame.from_dict(_sum_df, orient='index').T.add_suffix('_Counter')], axis=1)
...
def past_solved_features(df):
    ...
    output = pd.DataFrame()
    target_cols = ['num_correct', 'num_incorrect', 'accuracy_group', 'hour']
...
def clean_title_m(df):
    ...
    if DAYS:
        for title in title_cols:
            for c in ['num_correct', 'num_incorrect', 'accuracy_group', 'hour']:
