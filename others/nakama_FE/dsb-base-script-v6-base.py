...
def run_kfold_lightgbm(param, train, test, folds, features, target, n_fold=5, categorical=[]):
    ...
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": oof}).to_csv(OUTPUT_DICT+'oof_lightgbm_soft.csv', index=False)
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": qwk_oof}).to_csv(OUTPUT_DICT+'oof_lightgbm_hard.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": predictions})\
                    .to_csv(OUTPUT_DICT+'predictions_lightgbm_soft.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": qwk_predictions})\
                    .to_csv(OUTPUT_DICT+'predictions_lightgbm_hard.csv', index=False)
...
def run_kfold_xgb(param, train, test, folds, features, target, n_fold=5):
    ...
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": oof}).to_csv(OUTPUT_DICT+'oof_xgboost_soft.csv', index=False)
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": qwk_oof}).to_csv(OUTPUT_DICT+'oof_xgboost_hard.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": predictions}).to_csv(OUTPUT_DICT+'predictions_xgboost_soft.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": qwk_predictions})\
                    .to_csv(OUTPUT_DICT+'predictions_xgboost_hard.csv', index=False)
...
def run_kfold_catboost(param, train, test, folds, features, target, n_fold=5, categorical=[]):
    ...
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": oof}).to_csv(OUTPUT_DICT+'oof_catboost_soft.csv', index=False)
    pd.DataFrame({f"{ID}": train[ID].values, f"{TARGET}": qwk_oof}).to_csv(OUTPUT_DICT+'oof_catboost_hard.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": predictions}).to_csv(OUTPUT_DICT+'predictions_catboost_soft.csv', index=False)
    pd.DataFrame({f"{ID}": test[ID].values, f"{TARGET}": qwk_predictions})\
                    .to_csv(OUTPUT_DICT+'predictions_catboost_hard.csv', index=False)
...
def extract_user_logs(df, df_labels):
    ...
    nunique_cols = ['event_id', 'game_session', 'timestamp', 'event_data', 'event_count', 'event_code', 
                    'title', 'type', 'world', 'date', 'month', 'hour', 'dayofweek', 'weekofyear', 'title_event_code']
            ...
            for i in range(len(tmp_df)):
                ...
                if len(tmp_past)==0:
                    ...
                    concat_df = pd.concat([nunique_df, 
                                           pd.DataFrame.from_dict(_sum_df, orient='index').T], axis=1)
                    sum_df = pd.concat([sum_df, concat_df], axis=0)
        else:
            for i in range(len(tmp_df)):
                if i==0:
                    tmp_past = tmp[(tmp_df.loc[i, 'timestamp'] - offsets.Day(days)) < tmp.timestamp]\
                ...
                if len(tmp_past)==0:
                    ...
                    for c in sum_cols:
                        _sum_df.update(Counter(tmp_past[c].values))
                    concat_df = pd.concat([nunique_df, 
                                           pd.DataFrame.from_dict(_sum_df, orient='index').T], axis=1).add_prefix(f'{days}day_')
                    sum_df = pd.concat([sum_df, concat_df], axis=0)
                ...
                if days==None:
                    ...
                    if len(tmp_past)!=0:
                        for c in target_cols:
                            tmp.loc[i, c + '_sum'] = tmp_past[c].values.sum()
                            tmp.loc[i, c + '_max'] = tmp_past[c].values.max()
                            tmp.loc[i, c + '_min'] = tmp_past[c].values.min()
                            tmp.loc[i, c + '_mean'] = tmp_past[c].values.mean()
                            tmp.loc[i, c + '_median'] = tmp_past[c].median()
                            tmp.loc[i, c + '_var'] = tmp_past[c].values.var()
                    ...
                    for t in title_cols:
                        tmp_past = tmp[tmp.title == t][tmp.timestamp < tmp.loc[i, 'timestamp']]
                        if len(tmp_past)!=0:
                            for c in target_cols:
                                tmp.loc[i, c + '_sum_' + t] = tmp_past[c].values.sum()
                                tmp.loc[i, c + '_max_' + t] = tmp_past[c].values.max()
                                tmp.loc[i, c + '_min_' + t] = tmp_past[c].values.min()
                                tmp.loc[i, c + '_mean_' + t] = tmp_past[c].values.mean()
                                tmp.loc[i, c + '_median_' + t] = tmp_past[c].median()
                                tmp.loc[i, c + '_var_' + t] = tmp_past[c].values.var()
                            tmp.loc[i, 'total_accuracy_' + t] = \
                            tmp.loc[i, 'num_correct_sum_' + t] / (tmp.loc[i, 'num_correct_sum_' + t] + tmp.loc[i, 'num_incorrect_sum_' + t])
                else:
                    tmp_past = tmp[(tmp.loc[i, 'timestamp'] - offsets.Day(days)) < tmp.timestamp][tmp.timestamp < tmp.loc[i, 'timestamp']]
                    if len(tmp_past)!=0:
                        for c in target_cols:
                            tmp.loc[i, c + f'_sum_{days}day'] = tmp_past[c].values.sum()
                            tmp.loc[i, c + f'_max_{days}day'] = tmp_past[c].values.max()
                            tmp.loc[i, c + f'_min_{days}day'] = tmp_past[c].values.min()
                            tmp.loc[i, c + f'_mean_{days}day'] = tmp_past[c].values.mean()
                            tmp.loc[i, c + f'_median_{days}day'] = tmp_past[c].median()
                            tmp.loc[i, c + f'_var_{days}day'] = tmp_past[c].values.var()
                    ...
                    # Bird Measurer, Cart Balancer, Cauldron Filler, Chest Sorter, and Mushroom Sorter
                    for t in title_cols:
                        tmp_past = tmp[(tmp.loc[i, 'timestamp'] - offsets.Day(days)) < tmp.timestamp][tmp.title == t][tmp.timestamp < tmp.loc[i, 'timestamp']]
                        if len(tmp_past)!=0:
                            for c in target_cols:
                                tmp.loc[i, c + f'_sum_{days}day_' + t] = tmp_past[c].values.sum()
                                tmp.loc[i, c + f'_max_{days}day_' + t] = tmp_past[c].values.max()
                                tmp.loc[i, c + f'_min_{days}day_' + t] = tmp_past[c].values.min()
                                tmp.loc[i, c + f'_mean_{days}day_' + t] = tmp_past[c].values.mean()
                                tmp.loc[i, c + f'_median_{days}day_' + t] = tmp_past[c].median()
                                tmp.loc[i, c + f'_var_{days}day_' + t] = tmp_past[c].values.var()
...
def clean_title_m(df):
    ...
    if DAYS:
        for title in title_cols:
            for c in ['accuracy_group']:
                for m in ['mean', 'max', 'min', 'median', 'sum', 'var']:
